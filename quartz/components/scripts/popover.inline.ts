...
// 미리보기를 띄우지 않을 링크.
//   .tag-link  — 태그. 열어 봤자 "이 태그가 붙은 글 목록"이라 미리 볼 게 없습니다.
//   .tag-nav   — 왼쪽 사이드바의 분야 목차. 위와 같은 이유.
//   .no-popover — 필요할 때 직접 붙이는 예외 클래스.
// 본문 안의 글 링크([[위키링크]] 등)에는 그대로 미리보기가 뜹니다.
const POPOVER_EXCLUDE = "a.tag-link, .tag-nav a, a.no-popover"

function setupPopovers() {
  const links = [...document.querySelectorAll("a.internal")].filter(
    (link) => !link.matches(POPOVER_EXCLUDE),
